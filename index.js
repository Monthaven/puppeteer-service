...
app.get('/scrape', async (req, res) => {
  ...
  try {
    ...
    // Launch browser with production-grade args
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.log(`[Puppeteer] ✓ Navigation successful (attempt ${attempt})`);
        break;
      } catch (navError) {
        lastError = navError;
        console.error(`[Puppeteer] ✗ Navigation failed (attempt ${attempt}):`, navError.message);
        if (attempt < 3) {
    ...
    const images = await page.evaluate(() => {
      ...
      return imgs.filter(img => 
        ...
        (img.url.match(/\.(jpg|jpeg|png|webp)/i) || img.url.includes('image'))
      );
    });
    ...
    console.log(`[Puppeteer] ✓ Found ${images.length} images in ${duration}ms`);
...
// Fallback - simple fetch without Puppeteer
app.get('/scrape-simple', async (req, res) => {
  ...
  try {
    ...
    res.json({ images, count: images.length, method: 'simple-fetch' });
  } catch (error) {
